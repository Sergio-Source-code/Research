...
# vim:ft=python
#
# (c) Patrick Merlot <patrick.merlot@gmail.com>
# primitive frontend to cmake - originally written for DALTON/LSDALTON projects by:
...
def parse_input():
    ...
    group.add_argument('--cublas',
            action='store_true',
            default=False,
            help='enable the CUBLAS GPU library [default: %(default)s]')
    group.add_argument('--collapse',
            action='store_true',
            default=False,
            help='enable COLLAPSE clause for reorderings [default: %(default)s]')
    ...
    group.add_argument('--blacs',
            nargs='?',
            action='store',
            choices=('intelmpi', 'openmpi', 'sgimpt'),
            default='intelmpi',
            help='specify the BLACS implementation; only used in combination with --scalapack [default: %(default)s]')
    group.add_argument('--scalasca',
            action='store_true',
            default=False,
            help='build using SCALASCA profiler mode [default: %(default)s]')
    group.add_argument('--vampirtrace',
            action='store_true',
            default=False,
            help='build using Vampirtrace instrumentation [default: %(default)s]')
    ...
    group.add_argument('--qmatrix',
            action='store_true',
            default=False,
            help='enable QMatrix external library [default: %(default)s]')
...
def gen_cmake_command(args):
    ...
    command += ' -DENABLE_GPU=%s'            % translate_cmake(args.gpu)
    command += ' -DENABLE_CUBLAS=%s'         % translate_cmake(args.cublas)
    command += ' -DENABLE_COLLAPSE=%s'       % translate_cmake(args.collapse)
    command += ' -DENABLE_CSR=%s'            % translate_cmake(args.csr)
    command += ' -DENABLE_SCALASCA=%s'       % translate_cmake(args.scalasca)
    command += ' -DENABLE_VAMPIRTRACE=%s'    % translate_cmake(args.vampirtrace)
    command += ' -DENABLE_TIMINGS=%s'        % translate_cmake(args.timings)
    command += ' -DENABLE_STATIC_LINKING=%s' % translate_cmake(args.static)
    command += ' -DENABLE_QMATRIX=%s'        % translate_cmake(args.qmatrix)

    command += ' -DENABLE_SCALAPACK=%s'      % translate_cmake(args.scalapack)
    if args.scalapack:
        command += ' -DBLACS_IMPLEMENTATION=%s' % args.blacs
    ...
    if args.gpu:
        command += ' -DENABLE_OPENACC=ON'
        command += ' -DENABLE_CUDA=ON'
